...
class DedupAggregation(AggregationInterface):
    ...
    def semantic_parse(self, query: str, context: List[str]) -> str:
        ...
        try:
            system_prompt = (
                "你是一名专业的战锤40K规则专家，具备精准的信息提取与分析能力。"
                "你的任务是基于提供的上下文资料，为用户提供准确、全面且有条理的回答。"
            )
            user_prompt = f"""
## 核心要求
1.  **严格基于上下文**：仅使用提供的所有上下文信息进行回答，不引入任何外部知识或进行推测。所有答案内容必须能直接追溯到提供的文档。
2.  **信息引用**：上下文包括核心规则、问答集以及**所有提供的单位数据表和技能描述**。在回答中，您可以引用和使用这些数据表中明确列出的单位名称、武器名称、技能名称及其具体效果。
3.  **完整性与整合**：务必**整合所有分散在不同章节、问答集和单位数据表中的相关信息**，确保答案的全面性和连贯性。特别注意规则之间的相互作用和优先级。
4.  **结构化回答**：根据问题的复杂度和相关信息的丰富程度，选择最合适的回答结构。先说明核心规则，再补充例外和细节。对于简单直接的问题，可以直接提供一个综合性的答案，无需强制使用所有子标题。
5.  **专业术语**: 使用战锤40K的标准术语和表达方式。
6.  **来源标注**: 每条要点末尾以 `[来源X]` 标注文档编号。
...
## 输出格式
直接输出结构化的答案，无需添加“问题：”、“回答：”等标签。确保答案完整、准确、易于理解，且严格基于提供的上下文信息。输出结构化的答案，无需添加"问题："、"回答："等标签。确保答案完整、准确、易于理解，且严格基于提供的上下文信息。
