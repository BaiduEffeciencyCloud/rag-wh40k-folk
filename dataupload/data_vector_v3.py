...
def save_chunks_to_json(chunks: List[Dict[str, Any]], output_dir: str = "chunks", source_file: str = None) -> str:
    """
    保存切片结果到JSON文件
    ...
    # 生成时间戳，格式为YYMMDD_HHMM
    timestamp = datetime.now().strftime("%y%m%d_%H%M")
    
    # 根据是否提供source_file生成不同的文件名
    if source_file:
        # 提取不带后缀的文件名
...
def main(test_file: str):
    ...
    # 保存结果
    json_path = save_chunks_to_json(optimized_chunks, source_file=os.path.basename(test_file))
    print(f"结果已保存：\nJSON文件：{json_path}\n切片完成！\n")
